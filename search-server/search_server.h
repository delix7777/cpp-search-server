...
{
    if (!all_of(stop_words_.begin(), stop_words_.end(), [](const std::string& stop_word) { return IsValidWord(stop_word); })) {
        throw std::invalid_argument("Invalide characters in stop words");
    }
}
...
std::vector<Document> SearchServer::FindTopDocuments(const std::string& raw_query, DocumentPredicate document_predicate) const {
    ...
    if ((int)matched_documents.size() > MAX_RESULT_DOCUMENT_COUNT) {
        matched_documents.resize((long long unsigned int)MAX_RESULT_DOCUMENT_COUNT);
    }
