    ...
    vector<Document> FindAllDocuments(const Query& query_words) const {
        ...
        for (const auto& plus_word : query_words.plus_words) {
            if (word_to_document_freqs_.count(plus_word) != 0) {
                for (const auto& [id, rel] : word_to_document_freqs_.at(plus_word)) { 
                    document_to_relevance[id] += rel * log(document_count_ * 1.0 / word_to_document_freqs_.at(plus_word).size());
                }
            }
        }
